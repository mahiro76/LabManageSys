...
def run_command(cmd, capture_output=True, check=False, input_text=None):
    """运行 shell 命令并返回 (returncode, stdout, stderr) 三元组。"""
    try:
        if capture_output:
...
def initialize_mysql(force: bool = False) -> bool:
    """初始化 MySQL 数据目录。force=True 时覆盖已存在的目录。"""
    state = load_state()
...
def set_root_password(force: bool = False) -> bool:
    """修改 MySQL root 密码为 config.py 中 NEW_ROOT_PASSWORD 指定的值。force=True 时强制重做。"""
    state = load_state()
...
def configure_mysql(force: bool = False) -> bool:
    """创建额外数据库 myapp 和用户 myuser（供其他应用使用）。force=True 时强制重做。"""
    state = load_state()
...
def full_setup() -> bool:
    """完整部署 MySQL：依次执行初始化 → 启动 → 改密 → 额外配置。返回 True 表示全部成功。"""
    steps = [
...
def force_step(step_name: str) -> bool:
    """强制重做指定步骤。step_name 为 "all" 时重置所有状态并重新执行完整部署。"""
    state = load_state()
    if step_name == "all":
